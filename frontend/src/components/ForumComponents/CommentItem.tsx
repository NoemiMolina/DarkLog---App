...
export const CommentItem: React.FC<CommentItemProps> = ({
  ...
  const isLiked = Array.isArray(comment.likes)
    ? comment.likes.some(
        (userId: any) => String(userId) === String(currentUserId),
      )
    : false;
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/forum/posts/${postId}/comments/${commentIdToUse}/replies`,
        {
          method: "POST",
          headers: {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-white text-sm font-medium">
                {comment.author?.UserPseudo || "User"}
              </p>
          ...
          {showReply && (
            <form onSubmit={handleReply} className="flex gap-2 w-full mt-2">
              ...
              <Button
                ...
                className="h-8 bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90"
              >
