...
const POPULAR_TAGS = [
  {
    name: "movies",
    icon: "#",
    color: "bg-red-500/20 hover:bg-red-500/40 border-red-500/50",
  },
  {
    name: "tv-shows",
    icon: "#",
      ...
      {hasMoreTags && (
        ...
        >
          <span>
            {showAllTags
              ? "Show less"
              : `See more (${allTags.length - INITIAL_TAGS_COUNT})`}
          </span>
          <ChevronDown
            size={16}
            className={`transition-transform duration-200 ${showAllTags ? "rotate-180" : ""}`}
          />
        </button>
      )}
