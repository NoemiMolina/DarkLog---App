...
export const getWatchlistById = async (req: Request, res: Response) => {
  try {
    ...
    const watchlist =
      await HomemadeWatchlist.findById(watchlistId).populate("movies");
...
export const addWatchlistToUser = async (req: Request, res: Response) => {
  try {
    ...
    if (
      user.SavedHomemadeWatchlists.some((w) => w.toString() === watchlistId)
    ) {
      return res.status(400).json({
...
export const rateWatchlist = async (req: Request, res: Response) => {
  try {
    ...
    const ratings = user.RatedMovies.map((r) => r.rating).filter(
      (r) => typeof r === "number",
    );
    if (ratings.length > 0) {
