...
export const addRatingToEpisode = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || typeof rating !== "number") {
      return res
        .status(400)
        .json({ message: "UserId and numeric rating are required" });
    }
    ...
    if (user) {
      const allUserRatings = tvShow.seasons
        ...
        .flatMap((e) => e.ratings)
        .filter(
          (r) => r.userId?.toString() === userId && typeof r.value === "number",
        )
        .map((r) => r.value as number);
    ...
    res
      .status(200)
      .json({ message: "Rating added/updated", episode, episodeAverage });
  } catch (err) {
    console.error(err);
...
export const getRandomTVShow = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching random TV show", error: err });
  }
};
...
export const getTVShowsByStyle = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching TV shows by style", error: err });
  }
};
