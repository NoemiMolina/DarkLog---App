...
export const publishPost = async (req: Request, res: Response) => {
  try {
    ...
    if (post.author.toString() !== userId) {
      return res
        .status(403)
        .json({ message: "Forbidden: You can only publish your own posts" });
    }
...
export const getPublishedPosts = async (req: Request, res: Response) => {
  try {
    const posts = await Forum.find({ published: true })
      ...
      .populate(
        "author",
        "username UserPseudo UserFirstName UserProfilePicture",
      )
      .populate(
        "comments.author",
        "username UserPseudo UserFirstName UserProfilePicture",
...
export const addReactionToComment = async (req: Request, res: Response) => {
  try {
    ...
    if (!["like", "dislike"].includes(type)) {
      return res
        .status(400)
        .json({ message: "Type must be 'like' or 'dislike'" });
    }
...
export const markNotificationAsRead = async (req: Request, res: Response) => {
  try {
    res
      .status(200)
      .json({ message: "Notification marked as read (not implemented)" });
  } catch (error) {
    res
      .status(500)
