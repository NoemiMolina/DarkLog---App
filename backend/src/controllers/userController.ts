...
export const updateProfileInfos = async (req: Request, res: Response) => {
  try {
    ...
    const { UserFirstName, UserLastName, UserPseudo, UserLocation, UserAge } =
      req.body;
    ...
    res
      .status(500)
      .json({ message: "Error while updating profile", error: err });
  }
};
...
export const updateProfilePicture = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error while updating profile picture", error: err });
  }
};
...
export const getUserProfile = async (req: Request, res: Response) => {
  try {
    ...
    const watchedTvShowsWithDetails = user.RatedTvShows.map(
      (ratedShow: any) => {
    ...
    const profileData = {
      ...
      savedHomemadeWatchlists: (user.SavedHomemadeWatchlists as any[]).map(
        (watchlist: any) => ({
          _id: watchlist._id,
          id: watchlist._id,
    ...
    res
      .status(500)
      .json({ message: "Error while fetching profile", error: String(err) });
  }
};
// ------------- SEARCH USERS
export const searchUsers = async (req: Request, res: Response) => {
  try {
    ...
    const users = await User.find({
      ...
      .select(
        "_id UserPseudo UserMail UserProfilePicture UserFirstName UserLastName",
      )
      .limit(10);
...
export const getPublicProfile = async (req: Request, res: Response) => {
  try {
    ...
    const top3MovieIds = Array.isArray(user.Top3Movies)
      ? user.Top3Movies.filter((id) => typeof id === "number")
      : [];
    const movieWatchlistIds = Array.isArray(user.MovieWatchlist)
      ? user.MovieWatchlist.filter((id) => typeof id === "number")
      : [];
    const tvShowWatchlistIds = Array.isArray(user.TvShowWatchlist)
      ? user.TvShowWatchlist.filter((id) => typeof id === "number")
      : [];
    const top3TvShowIds = Array.isArray(user.Top3TvShow)
      ? user.Top3TvShow.filter((id) => typeof id === "number")
      : [];
    let top3Movies: any[] = [];
    ...
    if (top3MovieIds.length > 0) {
      try {
        top3Movies = await Movie.find({
          tmdb_id: { $in: top3MovieIds },
        }).lean();
      } catch (err) {
    ...
    if (movieWatchlistIds.length > 0) {
      try {
        movieWatchlist = await Movie.find({
          tmdb_id: { $in: movieWatchlistIds },
        }).lean();
      } catch (err) {
    ...
    if (top3TvShowIds.length > 0) {
      try {
        top3TvShows = await TVShow.find({
          tmdb_id: { $in: top3TvShowIds },
        }).lean();
      } catch (err) {
    ...
    if (tvShowWatchlistIds.length > 0) {
      try {
        tvShowWatchlist = await TVShow.find({
          tmdb_id: { $in: tvShowWatchlistIds },
        }).lean();
      } catch (err) {
    ...
    const watchedMoviesWithDetails = (
      Array.isArray(user.RatedMovies) ? user.RatedMovies : []
    ).map((ratedMovie: any) => {
      return { runtime: ratedMovie.runtime || 0 };
    });

    const watchedTvShowsWithDetails = (
      Array.isArray(user.RatedTvShows) ? user.RatedTvShows : []
    ).map((ratedShow: any) => {
      return { total_runtime: ratedShow.total_runtime || 0 };
    });
...
export const getFriends = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findById(userId).populate(
      "Friends",
      "UserPseudo UserMail UserProfilePicture UserFirstName UserLastName",
    );
...
export const addAFriend = async (req: Request, res: Response) => {
  try {
    ...
    user.Friends = user.Friends.filter((f) => f.friendId && f.friendPseudo);
    const isAlreadyFriend = user.Friends.some(
      (f) => f.friendId && f.friendId.toString() === friendId,
    );
    if (!isAlreadyFriend) {
      user.Friends.push({
...
export const deleteAFriend = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error while deleting a friend", error: err });
  }
};
...
export const getFriendsReviews = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findById(userId).populate(
      "Friends.friendId",
      "UserPseudo UserFirstName UserProfilePicture RatedMovies RatedTvShows",
    );
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
...
export const addAMovieToWatchlist = async (req: Request, res: Response) => {
  try {
    ...
    user.RatedMovies = user.RatedMovies.filter(
      (r: any) => r.tmdbMovieId && r.movieTitle,
    );

    user.RatedTvShows = user.RatedTvShows.filter(
      (r: any) => r.tmdbTvShowId && r.tvShowTitle,
    );
...
export const deleteAMovieFromWatchlist = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const movieTmdbId = Number(movieId);
    user.MovieWatchlist = user.MovieWatchlist.filter(
      (id) => id !== movieTmdbId,
    );
    await user.save();
...
export const deleteATvShowFromWatchlist = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const tvShowTmdbId = Number(tvShowId);
    user.TvShowWatchlist = user.TvShowWatchlist.filter(
      (id) => id !== tvShowTmdbId,
    );
    await user.save();
...
export const deleteAHomemadeWatchlistFromSavedWatchlists = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const addAMovieToTop3Favorites = async (req: Request, res: Response) => {
  try {
    ...
    if (currentTop3.length >= 3) {
      return res
        .status(400)
        .json({ message: "You can only have 3 favorite movies" });
    }
...
export const addATvShowToTop3Favorites = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (currentTop3.length >= 3) {
      return res
        .status(400)
        .json({ message: "You can only have 3 favorite TV Shows" });
    }
...
export const deleteAMovieFromTop3Favorites = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const movieTmdbId = Number(movieId);
    const currentTop3 = Array.isArray(user.Top3Movies) ? user.Top3Movies : [];
    const updatedTop3 = currentTop3.filter(
      (id: any) => Number(id) !== movieTmdbId,
    );
...
export const deleteATvShowFromTop3Favorites = async (
  req: Request,
  res: Response,
) => {
  try {
  ...
  try {
    ...
    if (type === "movie") {
      ...
      if (existingIndex !== -1) {
        ...
        ratedMovies[existingIndex].movieTitle =
          itemTitle || ratedMovies[existingIndex].movieTitle;
      ...
      if (existingIndex !== -1) {
        ...
        ratedTvShows[existingIndex].tvShowTitle =
          itemTitle || ratedTvShows[existingIndex].tvShowTitle;
